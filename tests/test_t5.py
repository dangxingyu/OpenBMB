...
def main():
    ...
    hug_t5 = hugT5.from_pretrained(path).cuda().eval().half()
    ...
    for _ in range(10):
        ...
        bmt_logits = bmt_t5(input_ids=input_ids, attention_mask=attention_mask, decoder_input_ids=decoder_input_ids,
                            decoder_attention_mask=decoder_attention_mask, return_logits=True)
        hug_logits = hug_t5(input_ids=input_ids, attention_mask=attention_mask,
